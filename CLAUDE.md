...
- **State transitions** for drafts (draft → sending → sent; the send is accepted synchronously and completed asynchronously: the fan-out settles to sent once at least one recipient was delivered to, a fully failed fan-out reverts to draft, and the zero-recipient edge case settles synchronously as sent).
