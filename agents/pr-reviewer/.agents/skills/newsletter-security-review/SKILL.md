...
description: >
  ...
  verification, gateway-delegated authorization, the HMAC-tokened
  unsubscribe, SQL construction, and secrets. Discovers the concrete guards
...
- **The unauthenticated surfaces.** The open-tracking pixel and the one-click
  unsubscribe are reachable by anyone, each guarded only by its own token (an
  opaque recipient hash; an HMAC-signed token). Weakening a token check,
...
- **Secrets and config.** Configuration is read in one documented place;
  secrets (DB credentials, the unsubscribe signing secret) must never appear
