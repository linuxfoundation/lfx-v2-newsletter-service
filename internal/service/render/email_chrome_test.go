...
func TestPreviewText(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "semantic containers get separators",
		...
		{
			name:     "title and head content never surface",
			bodyHTML: "<title>Doc title</title><p>Hi members</p>",
			want:     "Hi members",
		},
		{
		...
		{
			name:     "comments never surface",
			bodyHTML: "<!-- internal note --><p>Hi members</p>",
			want:     "Hi members",
		},
	}
	...
	t.Run("long body truncates at a word boundary with ellipsis", func(t *testing.T) {
		got := previewText("<p>" + strings.Repeat("lorem ipsum ", 20) + "</p>")
		if !strings.HasSuffix(got, "…") {
			t.Fatalf("previewText() = %q, want ellipsis suffix", got)
		}
		if n := len([]rune(got)); n > previewTextMaxLen {
			t.Errorf("previewText() length = %d runes including ellipsis, want <= %d", n, previewTextMaxLen)
		}
	...
	t.Run("unbroken long word stays within the cap including ellipsis", func(t *testing.T) {
		got := previewText("<p>" + strings.Repeat("a", 3*previewTextMaxLen) + "</p>")
		if !strings.HasSuffix(got, "…") {
			t.Fatalf("previewText() = %q, want ellipsis suffix", got)
		}
		if n := len([]rune(got)); n > previewTextMaxLen {
	...
	t.Run("body exactly at the cap is not truncated", func(t *testing.T) {
		exact := strings.Repeat("a", previewTextMaxLen)
		if got := previewText("<p>" + exact + "</p>"); got != exact {
			t.Errorf("previewText() = %q (len %d), want untouched %d-rune body", got, len([]rune(got)), previewTextMaxLen)
		}
	})
}

func TestEmailHTMLPreheader(t *testing.T) {
	t.Run("hidden preview sits immediately after the body tag", func(t *testing.T) {
		...
		if bodyIdx == -1 {
			t.Fatal("EmailHTML() missing <body>")
		}
		afterBody := html[bodyIdx+strings.Index(html[bodyIdx:], ">")+1:]
		if !strings.HasPrefix(strings.TrimLeft(afterBody, "\n"), `<div style="display:none;`) {
			t.Errorf("EmailHTML() preheader div must be the first element after <body>, got %q", afterBody[:80])
		}
		...
		preheaderDiv := afterBody[:strings.Index(afterBody, "</div>")]
		for _, frag := range []string{"display:none", "max-height:0", "overflow:hidden", "mso-hide:all"} {
			if !strings.Contains(preheaderDiv, frag) {
				t.Errorf("preheader container missing hiding fallback %q", frag)
			}
		}
		if got := strings.Count(preheaderDiv, "&nbsp;&zwnj;"); got < previewTextMaxLen {
			t.Errorf("preheader padding = %d &nbsp;&zwnj; pairs, want >= %d so a short preview fills the whole client window and body text is not appended", got, previewTextMaxLen)
		}
	})
