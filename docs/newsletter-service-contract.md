...
| `POST` | `/newsletters/sendgrid/events` | no | SendGrid signed event webhook (delivered / open / bounce / dropped / spamreport / blocked). Authenticity is the ECDSA signature over `timestamp + body` plus a 10-minute freshness window against replay; no user session. Registered only when `EMAIL_PROVIDER=sendgrid` and `SENDGRID_WEBHOOK_PUBLIC_KEY` is set. Returns `204`; `400` on a malformed body, `401` on a bad signature or stale timestamp. Gateway routing for this path (`httproute.yaml` / `ruleset.yaml`) is a pending chart follow-up. |
