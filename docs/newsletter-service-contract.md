...
- **`scheduled`** means at least one recipient's message has been accepted by the provider and is held for release at `scheduled_at`; the provider — not this service — owns the actual delivery timing. A periodic sweep (the same one that recovers stuck `sending` rows) flips `scheduled` rows to `sent` once `scheduled_at` has passed — this is reconciliation of our own display state only.
