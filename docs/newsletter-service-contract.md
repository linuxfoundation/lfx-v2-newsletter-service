...
| `GET` | `/projects/{project_uid}/newsletters/{newsletter_uid}/public` | no | Public "View Online" read for a sent newsletter, backing the footer link in the email chrome. There is no session, because a recipient opens it from their mail client. It only serves a newsletter whose body has already been dispatched to recipients, which covers `status='sent'`, a `sending` row that is not holding a future release time, and a `scheduled` row whose `scheduled_at` has passed. A draft, a send still holding a future release time, and a `newsletter_uid` or `project_uid` mismatch all return `404`, so the response never distinguishes "wrong project" from "not sent yet". Returns `PublicNewsletterView` (`subject`, `body_html`, `project_name`, `sent_at`) with `Cache-Control: public, max-age=300`. `sent_at` is absent until the row settles to `sent`. |
