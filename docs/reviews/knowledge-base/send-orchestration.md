...
**Detect:** in `internal/service/send_orchestrator.go` and the NATS clients, for any helper that validates or derives part of a value the peer also parses, confirm the implementation matches the peer's documented parsing step for step — not merely its purpose. Address-domain derivation must follow the peer's `mail.ParseAddress` then `strings.SplitN(addr.Address, "@", 2)` order rather than splitting the raw string at the last `@`. Read `lfx-v2-email-service/docs/email-service-contract.md` for the peer's rule; if that repo is not available locally, do not guess it.
