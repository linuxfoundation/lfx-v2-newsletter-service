...
- The per-recipient analytics endpoint (`GET /projects/{project_uid}/newsletters/{newsletter_uid}/analytics/recipients`) returns PII (recipient email addresses and per-recipient engagement) and is **always fail-closed**: it uses direct `openfga_check` with the `viewer` role and does NOT have an `allow_all` fallback. This route is unreachable when `openfga.enabled=false` — that is intentional for PII security.
