...
- The open pixel (`…/newsletter-opens/{newsletter_uid}`) and `/newsletters/unsubscribe` are intentionally unauthenticated because email clients request them without a user session (the unsubscribe link is authorized by its HMAC token). `/newsletters/unsubscribe` allows both `GET` (renders the read-only confirmation page) and `POST` (performs the opt-out mutation); `HEAD` is deliberately excluded.
