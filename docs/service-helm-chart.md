...
| `app.send.schedule.cancelBuffer` | `NEWSLETTER_SCHEDULE_CANCEL_BUFFER` | Rejects a cancel-schedule request inside this window before the armed `scheduled_at` (default `5m`) — SendGrid cancel is best-effort near release. |
