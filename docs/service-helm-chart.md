...
- The public "View Online" read (`GET /projects/{project_uid}/newsletters/{newsletter_uid}/public`, LFXV2-2579) is intentionally unauthenticated with an `allow_all` rule: a recipient opens the permalink from their mail client with no session. It is safe without FGA because the handler serves only rows scoped to the path `project_uid` whose body has already been dispatched to recipients, collapsing every other case to an indistinguishable `404`, and returns the narrow `PublicNewsletterView` projection (no PII). Dispatched covers `status='sent'`, plus the window in which the send has been committed but the row has not settled yet, because the link is embedded in the email before the fan-out completes. A draft, and a send still holding a future release time, are never served. It routes through the existing `^/projects/[^/]+/newsletters(/.*)?$` HTTPRoute regex — no new `httproute.yaml` entry is required.
