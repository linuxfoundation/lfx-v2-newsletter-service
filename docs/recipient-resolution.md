...
- **Per-recipient placeholder substitution.** Both paths run the rendered body (HTML and text) through `substitutePlaceholders` before each send. The legacy chrome footer only ever carries `%%UNSUBSCRIBE_URL%%`; layout emails additionally carry `%%MANAGE_SUBSCRIPTIONS_URL%%` and `%%VIEW_ONLINE_URL%%`. `%%UNSUBSCRIBE_URL%%` and `%%MANAGE_SUBSCRIPTIONS_URL%%` both resolve to the recipient's signed unsubscribe link (there is no distinct preferences surface today); `%%VIEW_ONLINE_URL%%` resolves to empty (no hosted web version yet), and the wrapper drops the View Online row when it is empty. When the unsubscribe service is not configured, the unsubscribe/manage sentinels are replaced with empty strings rather than left as raw `%%…%%` text. The layout-only sentinels are no-ops on the legacy path.
