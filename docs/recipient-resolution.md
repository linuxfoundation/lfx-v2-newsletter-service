...
- Systemic-outage fail-fast: once any recipient exhausts its attempts on a NATS **no-responders** condition, email-service is provably not subscribed to `lfx.email-service.send_email`, so the fan-out declares an outage and skips every recipient it has not started, recording them as failed with `skipped: email-service unavailable` (distinct from a dispatch error: we never tried). Dispatches already in flight finish normally, and the outage is scoped to the one fan-out — the next send resolves fresh. Deliberate design: no circuit breaker or half-open probing (a one-shot batch fan-out, and no-responders is a server-side fact rather than a statistical signal, so probing adds machinery without information); the trip condition is a single post-retry exhaustion and is intentionally not configurable (the signal is binary and authoritative — a threshold or env knob would only re-spend attempts on a subject known to be dead); and aborting early beats pressing on, because a totally-failed fan-out reverts the row to `draft` (cleanly re-sendable), whereas partial delivery is the one state with no clean recovery — there is no per-recipient dedup, so re-sending would double-send the delivered slice.
